...
class InputWindow:
    def __init__(self, root, obj_type, is_line, is_polyline, display_list, update_viewport):
        self.root = root
    ...
    def init_window(self):
        ...
        Label(input_window, text="Nome do objeto:").grid(row=0, column=0, padx=10, pady=5)
        self.name_entry = Entry(input_window)
        self.name_entry.grid(row=0, column=1, padx=10, pady=5)

        Label(input_window, text="Coordenada X1:").grid(row=1, column=0, padx=10, pady=5)
        self.x1_entry = Entry(input_window)
        self.x1_entry.grid(row=1, column=1, padx=10, pady=5)

        Label(input_window, text="Coordenada Y1:").grid(row=2, column=0, padx=10, pady=5)
        self.y1_entry = Entry(input_window)
        self.y1_entry.grid(row=2, column=1, padx=10, pady=5)

        if self.is_line:
            Label(input_window, text="Coordenada X2:").grid(row=3, column=0, padx=10, pady=5)
            self.x2_entry = Entry(input_window)
            self.x2_entry.grid(row=3, column=1, padx=10, pady=5)

            Label(input_window, text="Coordenada Y2:").grid(row=4, column=0, padx=10, pady=5)
            self.y2_entry = Entry(input_window)
            self.y2_entry.grid(row=4, column=1, padx=10, pady=5)

            Button(input_window, text="Adicionar Linha", command=self.add_point).grid(row=5, column=0, columnspan=2, pady=10)
        else:
            Button(input_window, text="Adicionar Ponto", command=self.add_point).grid(row=3, column=0, columnspan=2, pady=10)

        Button(input_window, text="Finalizar", command=self.finish).grid(row=6, column=0, columnspan=2, pady=10)
    ...
    def finish(self):
        ...
        if not self.is_line and len(self.points) < 2 and self.obj_type == "Polilinha":
            messagebox.showerror("Erro", "Polilinha precisa de pelo menos 2 pontos")
        elif not self.is_line and len(self.points) < 3 and self.obj_type == "Polígono":
