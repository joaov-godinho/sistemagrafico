...
class Viewport:
    ...
    # Função de transformação de coordenadas do mundo para o viewport (agora com 'z' opcional)
    def world_to_viewport(self, x, y, z=0):
    ...
    def update_canvas(self, display_list):
        ...
        for obj in display_list:
            if obj.obj_type == "Ponto":
                x, y, z = obj.coords[0]  # Extraímos Z, mas ele é ignorado
                x_vp, y_vp = self.world_to_viewport(x, y)
                self.canvas.create_oval(x_vp-2, y_vp-2, x_vp+2, y_vp+2, fill="black")
                self.canvas.create_text(x_vp + 10, y_vp, text=obj.name, anchor=tk.W)
            elif obj.obj_type == "Linha":
                if len(obj.coords) >= 2:
                    (x1, y1, z1), (x2, y2, z2) = obj.coords[:2]
